...
public class Music extends Thread {
	...
	public Music(boolean isLoop) { // 곡의 제목,무한반복여부
		try {
			this.isLoop = isLoop;
			file = new File("Buttercup.mp3");
			fis = new FileInputStream(file); // 해당 음악파일을 불러온다
			bis = new BufferedInputStream(fis); // 불러온 음악파일을 버퍼에 담는다
			player = new Player(bis);
			
			//player = new Player(new BufferedInputStream(new FileInputStream(new File("Buttercup.wav"))));
